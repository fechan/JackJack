...
var isDecimalRe = /^\s*(\+|-)?((\d+([,\.]\d+)?)|([,\.]\d+))\s*$/,
  ...
  parse: function(input, delimiterType, decimalSign, headersIncluded, safeHeaders, downcaseHeaders, upcaseHeaders, rowDelimiter) {
    var columnDelimiter,
      dataArray = [],
      errors = [];
    ...
    if (input.indexOf(columnDelimiter) > -1) {
      var re = new RegExp('^' + rowDelimiter + '+|' + rowDelimiter + '+$', 'g');
